...
class ContrailIntentMap(contrail.ContrailResource):
    ...
    properties_schema = {
        ...
        INTENT_MAP_INTENT_TYPE: properties.Schema(
            properties.Schema.STRING,
            _('INTENT_MAP_INTENT_TYPE.'),
            update_allowed=True,
            required=False,
        ),
    ...
    attributes_schema = {
        ...
        INTENT_MAP_INTENT_TYPE: attributes.Schema(
            _('INTENT_MAP_INTENT_TYPE.'),
        ),
    ...
    def handle_create(self):
        ...
        obj_0 = vnc_api.IntentMap(name=self.properties[self.NAME],
            parent_obj=parent_obj)

        if self.properties.get(self.INTENT_MAP_INTENT_TYPE) is not None:
            obj_0.set_intent_map_intent_type(self.properties.get(self.INTENT_MAP_INTENT_TYPE))
        ...
        try:
            obj_uuid = super(ContrailIntentMap, self).resource_create(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        try:
            obj_0 = self.vnc_lib().intent_map_read(
                id=self.resource_id
            )
        except Exception as e:
            raise Exception(_('%s') % str(e))

        if prop_diff.get(self.INTENT_MAP_INTENT_TYPE) is not None:
            obj_0.set_intent_map_intent_type(prop_diff.get(self.INTENT_MAP_INTENT_TYPE))
        ...
        try:
            self.vnc_lib().intent_map_update(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_delete(self):
        ...
        try:
            self.vnc_lib().intent_map_delete(id=self.resource_id)
        except Exception as ex:
            self._ignore_not_found(ex)
            LOG.warn(_('intent_map %s already deleted.') % self.name)

    @contrail.set_auth_token
    def _show_resource(self):
        obj = self.vnc_lib().intent_map_read(id=self.resource_id)
...
def resource_mapping():
    return {
        'OS::ContrailV2::IntentMap': ContrailIntentMap,
    }
