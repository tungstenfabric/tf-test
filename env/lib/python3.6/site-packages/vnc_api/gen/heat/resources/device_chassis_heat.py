...
class ContrailDeviceChassis(contrail.ContrailResource):
    ...
    properties_schema = {
        ...
        DEVICE_CHASSIS_TYPE: properties.Schema(
            properties.Schema.STRING,
            _('DEVICE_CHASSIS_TYPE.'),
            update_allowed=True,
            required=False,
        ),
    ...
    attributes_schema = {
        ...
        DEVICE_CHASSIS_TYPE: attributes.Schema(
            _('DEVICE_CHASSIS_TYPE.'),
        ),
    ...
    def handle_create(self):
        obj_0 = vnc_api.DeviceChassis(name=self.properties[self.NAME])

        if self.properties.get(self.DEVICE_CHASSIS_TYPE) is not None:
            obj_0.set_device_chassis_type(self.properties.get(self.DEVICE_CHASSIS_TYPE))
        ...
        try:
            obj_uuid = super(ContrailDeviceChassis, self).resource_create(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        try:
            obj_0 = self.vnc_lib().device_chassis_read(
                id=self.resource_id
            )
        except Exception as e:
            raise Exception(_('%s') % str(e))

        if prop_diff.get(self.DEVICE_CHASSIS_TYPE) is not None:
            obj_0.set_device_chassis_type(prop_diff.get(self.DEVICE_CHASSIS_TYPE))
        ...
        try:
            self.vnc_lib().device_chassis_update(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_delete(self):
        ...
        try:
            self.vnc_lib().device_chassis_delete(id=self.resource_id)
        except Exception as ex:
            self._ignore_not_found(ex)
            LOG.warn(_('device_chassis %s already deleted.') % self.name)

    @contrail.set_auth_token
    def _show_resource(self):
        obj = self.vnc_lib().device_chassis_read(id=self.resource_id)
...
def resource_mapping():
    return {
        'OS::ContrailV2::DeviceChassis': ContrailDeviceChassis,
    }
