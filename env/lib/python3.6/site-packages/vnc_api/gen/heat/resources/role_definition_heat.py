...
class ContrailRoleDefinition(contrail.ContrailResource):
    ...
    properties_schema = {
        ...
        FEATURE_REFS: properties.Schema(
            properties.Schema.LIST,
            _('FEATURE_REFS.'),
            update_allowed=True,
            required=False,
    ...
    attributes_schema = {
        ...
        FEATURE_REFS: attributes.Schema(
            _('FEATURE_REFS.'),
        ),
    ...
    def handle_create(self):
        ...
        obj_0 = vnc_api.RoleDefinition(name=self.properties[self.NAME],
        ...
        # reference to feature_refs
        if self.properties.get(self.FEATURE_REFS):
            for index_0 in range(len(self.properties.get(self.FEATURE_REFS))):
                try:
                    ref_obj = self.vnc_lib().feature_read(
                        id=self.properties.get(self.FEATURE_REFS)[index_0]
                    )
                except vnc_api.NoIdError:
                    ref_obj = self.vnc_lib().feature_read(
                        fq_name_str=self.properties.get(self.FEATURE_REFS)[index_0]
                    )
                except Exception as e:
                    raise Exception(_('%s') % str(e))
                obj_0.add_feature(ref_obj)
        ...
        try:
            obj_uuid = super(ContrailRoleDefinition, self).resource_create(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        try:
            obj_0 = self.vnc_lib().role_definition_read(
                id=self.resource_id
            )
        except Exception as e:
            raise Exception(_('%s') % str(e))
        ...
        # reference to feature_refs
        ref_obj_list = []
        if self.FEATURE_REFS in prop_diff:
            for index_0 in range(len(prop_diff.get(self.FEATURE_REFS) or [])):
                try:
                    ref_obj = self.vnc_lib().feature_read(
                        id=prop_diff.get(self.FEATURE_REFS)[index_0]
                    )
                except vnc_api.NoIdError:
                    ref_obj = self.vnc_lib().feature_read(
                        fq_name_str=prop_diff.get(self.FEATURE_REFS)[index_0]
                    )
                except Exception as e:
                    raise Exception(_('%s') % str(e))
                ref_obj_list.append({'to':ref_obj.fq_name})

            obj_0.set_feature_list(ref_obj_list)
            # End: reference to feature_refs
        ...
        try:
            self.vnc_lib().role_definition_update(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_delete(self):
        ...
        try:
            self.vnc_lib().role_definition_delete(id=self.resource_id)
        except Exception as ex:
            self._ignore_not_found(ex)
            LOG.warn(_('role_definition %s already deleted.') % self.name)

    @contrail.set_auth_token
    def _show_resource(self):
        obj = self.vnc_lib().role_definition_read(id=self.resource_id)
...
def resource_mapping():
    return {
        'OS::ContrailV2::RoleDefinition': ContrailRoleDefinition,
    }
