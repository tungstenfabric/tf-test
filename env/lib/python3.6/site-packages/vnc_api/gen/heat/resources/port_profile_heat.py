...
class ContrailPortProfile(contrail.ContrailResource):
    ...
    properties_schema = {
        ...
        STORM_CONTROL_PROFILE_REFS: properties.Schema(
            properties.Schema.LIST,
            _('STORM_CONTROL_PROFILE_REFS.'),
            update_allowed=True,
            required=False,
        ),
    ...
    attributes_schema = {
        ...
        STORM_CONTROL_PROFILE_REFS: attributes.Schema(
            _('STORM_CONTROL_PROFILE_REFS.'),
        ),
    ...
    def handle_create(self):
        ...
        obj_0 = vnc_api.PortProfile(name=self.properties[self.NAME],
        ...
        # reference to storm_control_profile_refs
        if self.properties.get(self.STORM_CONTROL_PROFILE_REFS):
            for index_0 in range(len(self.properties.get(self.STORM_CONTROL_PROFILE_REFS))):
                try:
                    ref_obj = self.vnc_lib().storm_control_profile_read(
                        id=self.properties.get(self.STORM_CONTROL_PROFILE_REFS)[index_0]
                    )
                except vnc_api.NoIdError:
                    ref_obj = self.vnc_lib().storm_control_profile_read(
                        fq_name_str=self.properties.get(self.STORM_CONTROL_PROFILE_REFS)[index_0]
                    )
                except Exception as e:
                    raise Exception(_('%s') % str(e))
                obj_0.add_storm_control_profile(ref_obj)

        try:
            obj_uuid = super(ContrailPortProfile, self).resource_create(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        try:
            obj_0 = self.vnc_lib().port_profile_read(
                id=self.resource_id
            )
        except Exception as e:
            raise Exception(_('%s') % str(e))
        ...
        # reference to storm_control_profile_refs
        ref_obj_list = []
        if self.STORM_CONTROL_PROFILE_REFS in prop_diff:
            for index_0 in range(len(prop_diff.get(self.STORM_CONTROL_PROFILE_REFS) or [])):
                try:
                    ref_obj = self.vnc_lib().storm_control_profile_read(
                        id=prop_diff.get(self.STORM_CONTROL_PROFILE_REFS)[index_0]
                    )
                except vnc_api.NoIdError:
                    ref_obj = self.vnc_lib().storm_control_profile_read(
                        fq_name_str=prop_diff.get(self.STORM_CONTROL_PROFILE_REFS)[index_0]
                    )
                except Exception as e:
                    raise Exception(_('%s') % str(e))
                ref_obj_list.append({'to':ref_obj.fq_name})

            obj_0.set_storm_control_profile_list(ref_obj_list)
            # End: reference to storm_control_profile_refs

        try:
            self.vnc_lib().port_profile_update(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_delete(self):
        ...
        try:
            self.vnc_lib().port_profile_delete(id=self.resource_id)
        except Exception as ex:
            self._ignore_not_found(ex)
            LOG.warn(_('port_profile %s already deleted.') % self.name)

    @contrail.set_auth_token
    def _show_resource(self):
        obj = self.vnc_lib().port_profile_read(id=self.resource_id)
...
def resource_mapping():
    return {
        'OS::ContrailV2::PortProfile': ContrailPortProfile,
    }
