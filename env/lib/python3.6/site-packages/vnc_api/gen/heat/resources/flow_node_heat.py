...
class ContrailFlowNode(contrail.ContrailResource):
    PROPERTIES = (
        NAME, FQ_NAME, FLOW_NODE_LOAD_BALANCER_IP, DISPLAY_NAME, PERMS2, PERMS2_OWNER, PERMS2_OWNER_ACCESS, PERMS2_GLOBAL_ACCESS, PERMS2_SHARE, PERMS2_SHARE_TENANT, PERMS2_SHARE_TENANT_ACCESS, FLOW_NODE_INBAND_INTERFACE, ID_PERMS, ID_PERMS_PERMISSIONS, ID_PERMS_PERMISSIONS_OWNER, ID_PERMS_PERMISSIONS_OWNER_ACCESS, ID_PERMS_PERMISSIONS_GROUP, ID_PERMS_PERMISSIONS_GROUP_ACCESS, ID_PERMS_PERMISSIONS_OTHER_ACCESS, ID_PERMS_UUID, ID_PERMS_UUID_UUID_MSLONG, ID_PERMS_UUID_UUID_LSLONG, ID_PERMS_ENABLE, ID_PERMS_CREATED, ID_PERMS_LAST_MODIFIED, ID_PERMS_DESCRIPTION, ID_PERMS_USER_VISIBLE, ID_PERMS_CREATOR, FLOW_NODE_IP_ADDRESS, ANNOTATIONS, ANNOTATIONS_KEY_VALUE_PAIR, ANNOTATIONS_KEY_VALUE_PAIR_KEY, ANNOTATIONS_KEY_VALUE_PAIR_VALUE, TAG_REFS, VIRTUAL_NETWORK_REFS, GLOBAL_SYSTEM_CONFIG
    ) = (
        'name', 'fq_name', 'flow_node_load_balancer_ip', 'display_name', 'perms2', 'perms2_owner', 'perms2_owner_access', 'perms2_global_access', 'perms2_share', 'perms2_share_tenant', 'perms2_share_tenant_access', 'flow_node_inband_interface', 'id_perms', 'id_perms_permissions', 'id_perms_permissions_owner', 'id_perms_permissions_owner_access', 'id_perms_permissions_group', 'id_perms_permissions_group_access', 'id_perms_permissions_other_access', 'id_perms_uuid', 'id_perms_uuid_uuid_mslong', 'id_perms_uuid_uuid_lslong', 'id_perms_enable', 'id_perms_created', 'id_perms_last_modified', 'id_perms_description', 'id_perms_user_visible', 'id_perms_creator', 'flow_node_ip_address', 'annotations', 'annotations_key_value_pair', 'annotations_key_value_pair_key', 'annotations_key_value_pair_value', 'tag_refs', 'virtual_network_refs', 'global_system_config'
    )
    ...
    properties_schema = {
        ...
        FLOW_NODE_LOAD_BALANCER_IP: properties.Schema(
            properties.Schema.STRING,
            _('FLOW_NODE_LOAD_BALANCER_IP.'),
            update_allowed=True,
            required=False,
        ),
        ...
        FLOW_NODE_INBAND_INTERFACE: properties.Schema(
            properties.Schema.STRING,
            _('FLOW_NODE_INBAND_INTERFACE.'),
            update_allowed=True,
            required=False,
        ),
        ...
        FLOW_NODE_IP_ADDRESS: properties.Schema(
            properties.Schema.STRING,
            _('FLOW_NODE_IP_ADDRESS.'),
            update_allowed=True,
            required=False,
        ),
    ...
    attributes_schema = {
        ...
        FLOW_NODE_LOAD_BALANCER_IP: attributes.Schema(
            _('FLOW_NODE_LOAD_BALANCER_IP.'),
        ),
        ...
        FLOW_NODE_INBAND_INTERFACE: attributes.Schema(
            _('FLOW_NODE_INBAND_INTERFACE.'),
        ),
        ID_PERMS: attributes.Schema(
            _('ID_PERMS.'),
        ),
        FLOW_NODE_IP_ADDRESS: attributes.Schema(
            _('FLOW_NODE_IP_ADDRESS.'),
        ),
    ...
    def handle_create(self):
        ...
        obj_0 = vnc_api.FlowNode(name=self.properties[self.NAME],
            parent_obj=parent_obj)

        if self.properties.get(self.FLOW_NODE_LOAD_BALANCER_IP) is not None:
            obj_0.set_flow_node_load_balancer_ip(self.properties.get(self.FLOW_NODE_LOAD_BALANCER_IP))
        ...
        if self.properties.get(self.FLOW_NODE_INBAND_INTERFACE) is not None:
            obj_0.set_flow_node_inband_interface(self.properties.get(self.FLOW_NODE_INBAND_INTERFACE))
        ...
        if self.properties.get(self.FLOW_NODE_IP_ADDRESS) is not None:
            obj_0.set_flow_node_ip_address(self.properties.get(self.FLOW_NODE_IP_ADDRESS))
        ...
        try:
            obj_uuid = super(ContrailFlowNode, self).resource_create(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        try:
            obj_0 = self.vnc_lib().flow_node_read(
                id=self.resource_id
            )
        except Exception as e:
            raise Exception(_('%s') % str(e))

        if prop_diff.get(self.FLOW_NODE_LOAD_BALANCER_IP) is not None:
            obj_0.set_flow_node_load_balancer_ip(prop_diff.get(self.FLOW_NODE_LOAD_BALANCER_IP))
        if prop_diff.get(self.DISPLAY_NAME) is not None:
            obj_0.set_display_name(prop_diff.get(self.DISPLAY_NAME))
        if prop_diff.get(self.FLOW_NODE_INBAND_INTERFACE) is not None:
            obj_0.set_flow_node_inband_interface(prop_diff.get(self.FLOW_NODE_INBAND_INTERFACE))
        if prop_diff.get(self.FLOW_NODE_IP_ADDRESS) is not None:
            obj_0.set_flow_node_ip_address(prop_diff.get(self.FLOW_NODE_IP_ADDRESS))
        ...
        try:
            self.vnc_lib().flow_node_update(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_delete(self):
        ...
        try:
            self.vnc_lib().flow_node_delete(id=self.resource_id)
        except Exception as ex:
            self._ignore_not_found(ex)
            LOG.warn(_('flow_node %s already deleted.') % self.name)

    @contrail.set_auth_token
    def _show_resource(self):
        obj = self.vnc_lib().flow_node_read(id=self.resource_id)
...
def resource_mapping():
    return {
        'OS::ContrailV2::FlowNode': ContrailFlowNode,
    }
