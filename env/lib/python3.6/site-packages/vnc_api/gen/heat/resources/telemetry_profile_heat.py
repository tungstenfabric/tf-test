...
class ContrailTelemetryProfile(contrail.ContrailResource):
    PROPERTIES = (
        NAME, FQ_NAME, DISPLAY_NAME, ID_PERMS, ID_PERMS_PERMISSIONS, ID_PERMS_PERMISSIONS_OWNER, ID_PERMS_PERMISSIONS_OWNER_ACCESS, ID_PERMS_PERMISSIONS_GROUP, ID_PERMS_PERMISSIONS_GROUP_ACCESS, ID_PERMS_PERMISSIONS_OTHER_ACCESS, ID_PERMS_UUID, ID_PERMS_UUID_UUID_MSLONG, ID_PERMS_UUID_UUID_LSLONG, ID_PERMS_ENABLE, ID_PERMS_CREATED, ID_PERMS_LAST_MODIFIED, ID_PERMS_DESCRIPTION, ID_PERMS_USER_VISIBLE, ID_PERMS_CREATOR, TELEMETRY_PROFILE_IS_DEFAULT, ANNOTATIONS, ANNOTATIONS_KEY_VALUE_PAIR, ANNOTATIONS_KEY_VALUE_PAIR_KEY, ANNOTATIONS_KEY_VALUE_PAIR_VALUE, PERMS2, PERMS2_OWNER, PERMS2_OWNER_ACCESS, PERMS2_GLOBAL_ACCESS, PERMS2_SHARE, PERMS2_SHARE_TENANT, PERMS2_SHARE_TENANT_ACCESS, TAG_REFS, SFLOW_PROFILE_REFS, PROJECT
    ) = (
        'name', 'fq_name', 'display_name', 'id_perms', 'id_perms_permissions', 'id_perms_permissions_owner', 'id_perms_permissions_owner_access', 'id_perms_permissions_group', 'id_perms_permissions_group_access', 'id_perms_permissions_other_access', 'id_perms_uuid', 'id_perms_uuid_uuid_mslong', 'id_perms_uuid_uuid_lslong', 'id_perms_enable', 'id_perms_created', 'id_perms_last_modified', 'id_perms_description', 'id_perms_user_visible', 'id_perms_creator', 'telemetry_profile_is_default', 'annotations', 'annotations_key_value_pair', 'annotations_key_value_pair_key', 'annotations_key_value_pair_value', 'perms2', 'perms2_owner', 'perms2_owner_access', 'perms2_global_access', 'perms2_share', 'perms2_share_tenant', 'perms2_share_tenant_access', 'tag_refs', 'sflow_profile_refs', 'project'
    )
    ...
    properties_schema = {
        ...
        TELEMETRY_PROFILE_IS_DEFAULT: properties.Schema(
            properties.Schema.BOOLEAN,
            _('TELEMETRY_PROFILE_IS_DEFAULT.'),
            update_allowed=True,
            required=False,
        ),
        ...
        SFLOW_PROFILE_REFS: properties.Schema(
            properties.Schema.LIST,
            _('SFLOW_PROFILE_REFS.'),
            update_allowed=True,
            required=False,
        ),
    ...
    attributes_schema = {
        ...
        TELEMETRY_PROFILE_IS_DEFAULT: attributes.Schema(
            _('TELEMETRY_PROFILE_IS_DEFAULT.'),
        ),
        ...
        SFLOW_PROFILE_REFS: attributes.Schema(
            _('SFLOW_PROFILE_REFS.'),
        ),
    ...
    def handle_create(self):
        ...
        obj_0 = vnc_api.TelemetryProfile(name=self.properties[self.NAME],
        ...
        if self.properties.get(self.TELEMETRY_PROFILE_IS_DEFAULT) is not None:
            obj_0.set_telemetry_profile_is_default(self.properties.get(self.TELEMETRY_PROFILE_IS_DEFAULT))
        ...
        # reference to sflow_profile_refs
        if self.properties.get(self.SFLOW_PROFILE_REFS):
            for index_0 in range(len(self.properties.get(self.SFLOW_PROFILE_REFS))):
                try:
                    ref_obj = self.vnc_lib().sflow_profile_read(
                        id=self.properties.get(self.SFLOW_PROFILE_REFS)[index_0]
                    )
                except vnc_api.NoIdError:
                    ref_obj = self.vnc_lib().sflow_profile_read(
                        fq_name_str=self.properties.get(self.SFLOW_PROFILE_REFS)[index_0]
                    )
                except Exception as e:
                    raise Exception(_('%s') % str(e))
                obj_0.add_sflow_profile(ref_obj)

        try:
            obj_uuid = super(ContrailTelemetryProfile, self).resource_create(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        try:
            obj_0 = self.vnc_lib().telemetry_profile_read(
                id=self.resource_id
            )
        except Exception as e:
            raise Exception(_('%s') % str(e))
        ...
        if prop_diff.get(self.TELEMETRY_PROFILE_IS_DEFAULT) is not None:
            obj_0.set_telemetry_profile_is_default(prop_diff.get(self.TELEMETRY_PROFILE_IS_DEFAULT))
        ...
        # reference to sflow_profile_refs
        ref_obj_list = []
        if self.SFLOW_PROFILE_REFS in prop_diff:
            for index_0 in range(len(prop_diff.get(self.SFLOW_PROFILE_REFS) or [])):
                try:
                    ref_obj = self.vnc_lib().sflow_profile_read(
                        id=prop_diff.get(self.SFLOW_PROFILE_REFS)[index_0]
                    )
                except vnc_api.NoIdError:
                    ref_obj = self.vnc_lib().sflow_profile_read(
                        fq_name_str=prop_diff.get(self.SFLOW_PROFILE_REFS)[index_0]
                    )
                except Exception as e:
                    raise Exception(_('%s') % str(e))
                ref_obj_list.append({'to':ref_obj.fq_name})

            obj_0.set_sflow_profile_list(ref_obj_list)
            # End: reference to sflow_profile_refs

        try:
            self.vnc_lib().telemetry_profile_update(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_delete(self):
        ...
        try:
            self.vnc_lib().telemetry_profile_delete(id=self.resource_id)
        except Exception as ex:
            self._ignore_not_found(ex)
            LOG.warn(_('telemetry_profile %s already deleted.') % self.name)

    @contrail.set_auth_token
    def _show_resource(self):
        obj = self.vnc_lib().telemetry_profile_read(id=self.resource_id)
...
def resource_mapping():
    return {
        'OS::ContrailV2::TelemetryProfile': ContrailTelemetryProfile,
    }
