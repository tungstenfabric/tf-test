...
class ContrailOverlayRole(contrail.ContrailResource):
    PROPERTIES = (
    ...
    def handle_create(self):
        ...
        obj_0 = vnc_api.OverlayRole(name=self.properties[self.NAME],
        ...
        try:
            obj_uuid = super(ContrailOverlayRole, self).resource_create(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        try:
            obj_0 = self.vnc_lib().overlay_role_read(
                id=self.resource_id
            )
        except Exception as e:
            raise Exception(_('%s') % str(e))
        ...
        try:
            self.vnc_lib().overlay_role_update(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_delete(self):
        ...
        try:
            self.vnc_lib().overlay_role_delete(id=self.resource_id)
        except Exception as ex:
            self._ignore_not_found(ex)
            LOG.warn(_('overlay_role %s already deleted.') % self.name)

    @contrail.set_auth_token
    def _show_resource(self):
        obj = self.vnc_lib().overlay_role_read(id=self.resource_id)
...
def resource_mapping():
    return {
        'OS::ContrailV2::OverlayRole': ContrailOverlayRole,
    }
