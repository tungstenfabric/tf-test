...
class ContrailHardwareInventory(contrail.ContrailResource):
    ...
    properties_schema = {
        ...
        HARDWARE_INVENTORY_INVENTORY_INFO: properties.Schema(
            properties.Schema.STRING,
            _('HARDWARE_INVENTORY_INVENTORY_INFO.'),
            update_allowed=True,
            required=False,
        ),
    ...
    attributes_schema = {
        ...
        HARDWARE_INVENTORY_INVENTORY_INFO: attributes.Schema(
            _('HARDWARE_INVENTORY_INVENTORY_INFO.'),
        ),
    ...
    def handle_create(self):
        ...
        obj_0 = vnc_api.HardwareInventory(name=self.properties[self.NAME],
            parent_obj=parent_obj)

        if self.properties.get(self.HARDWARE_INVENTORY_INVENTORY_INFO) is not None:
            obj_0.set_hardware_inventory_inventory_info(self.properties.get(self.HARDWARE_INVENTORY_INVENTORY_INFO))
        ...
        try:
            obj_uuid = super(ContrailHardwareInventory, self).resource_create(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        try:
            obj_0 = self.vnc_lib().hardware_inventory_read(
                id=self.resource_id
            )
        except Exception as e:
            raise Exception(_('%s') % str(e))

        if prop_diff.get(self.HARDWARE_INVENTORY_INVENTORY_INFO) is not None:
            obj_0.set_hardware_inventory_inventory_info(prop_diff.get(self.HARDWARE_INVENTORY_INVENTORY_INFO))
        ...
        try:
            self.vnc_lib().hardware_inventory_update(obj_0)
        except Exception as e:
            raise Exception(_('%s') % str(e))
    ...
    def handle_delete(self):
        ...
        try:
            self.vnc_lib().hardware_inventory_delete(id=self.resource_id)
        except Exception as ex:
            self._ignore_not_found(ex)
            LOG.warn(_('hardware_inventory %s already deleted.') % self.name)

    @contrail.set_auth_token
    def _show_resource(self):
        obj = self.vnc_lib().hardware_inventory_read(id=self.resource_id)
...
def resource_mapping():
    return {
        'OS::ContrailV2::HardwareInventory': ContrailHardwareInventory,
    }
