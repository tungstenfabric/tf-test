...
class EtreeToDict(object):
    ...
    def get_all_entry(self, path):
        ...
        self.logger.info("[get_all_entry] xps = %s" % str(xps))
        if not xps:
            # sometime ./xpath dosen't work; work around
            # should debug to find the root cause.
            self.logger.info("[get_all_entry]  # should debug to find the root cause????")
            xps = path.xpath(self.xpath.strip('.'))
            self.logger.info("[get_all_entry]  xps = %s" % str(xps))
        if type(xps) is not list:
            self.logger.info("[get_all_entry]  type(xps) == %s, return dict?" % str(type(xps)))
            return self._get_one(xps)

        val = []
        for xp in xps:
            val.append(self._get_one(xp))
        self.logger.info("[get_all_entry]  return len = %s, type(len(val[0])) == %s" % (str(len(val)), str(type(len(val[0])))))
        if len(val) == 1:
            return val[0]
        return val
