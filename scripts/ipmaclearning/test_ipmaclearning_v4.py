...
from .base import BaseIpMacLearningTest
...
class TestIpMacLearning(BaseVrouterTest,BaseIpMacLearningTest, BaseHC):
    @classmethod
    def setUpClass(cls):
        super(TestIpMacLearning, cls).setUpClass()

    @classmethod
    def tearDownClass(cls):
        super(TestIpMacLearning, cls).tearDownClass()
    ...
    def setUp(self):
        ...
        '''
        super(TestIpMacLearning, self).setUp()
        ...
        self.vm1_fixture = self.create_vm(vn_fixture=self.vn1_fixture, image_name='ubuntu', vm_name=self.vm1_name, node_name=self.node1)
        self.vm2_fixture = self.create_vm(vn_fixture=self.vn2_fixture, image_name='ubuntu', vm_name=self.vm2_name, node_name=self.node2)
        self.vm3_fixture = self.create_vm(vn_fixture=self.vn2_fixture, image_name='ubuntu', vm_name=self.vm3_name, node_name=self.node1)
        self.vm4_fixture = self.create_vm(vn_fixture=self.vn1_fixture, image_name='ubuntu', vm_name=self.vm4_name, node_name=self.node1)
        ...
        self.vm1_macvlan_ip = ".".join(self.vm1_eth0_ip.split(".")[:3]) + "." + str(int(self.vm1_eth0_ip.split(".")[3])+5) + "/32" 
        self.vm2_macvlan_ip = ".".join(self.vm2_eth0_ip.split(".")[:3]) + "." + str(int(self.vm2_eth0_ip.split(".")[3])+5) + "/32"
        self.vm3_macvlan_ip = ".".join(self.vm3_eth0_ip.split(".")[:3]) + "." + str(int(self.vm3_eth0_ip.split(".")[3])+5) + "/32"
        self.vm4_macvlan_ip = ".".join(self.vm4_eth0_ip.split(".")[:3]) + "." + str(int(self.vm4_eth0_ip.split(".")[3])+5) + "/32"
    ...
    def test_maciplearning_flag(self):
        ...
        flag = self.inputs.run_cmd_on_server(vm1_node_ip, flag_cmd).split(":")[1]
    ...
    def test_intra_vn_intra_compute_l2(self):
        ...
        vm1_macvlan_mac_addr = list(self.vm1_fixture.run_cmd_on_vm(mac_cmd).values())[0]
        vm4_macvlan_mac_addr = list(self.vm4_fixture.run_cmd_on_vm(mac_cmd).values())[0]
        ...
        try:
            ...
            mac = None
        assert not mac ,"Mac route for macvlan intf of vm4 is present in vm1 EVPN table."

        mac = self.agent_inspect[vm1_node_ip].get_vna_evpn_route(vm1_vrf_id, vxlanid=self.vn1_vxlan_id , mac=vm4_macvlan_mac_addr, ip="0.0.0.0/32")['mac']
        assert  mac == str(self.vn1_vxlan_id)+"-"+vm4_macvlan_mac_addr+"-"+"0.0.0.0/32", "Mac route for macvlan intf of vm4 is absent in vm1 EVPN table. "

        # checking bridge table
        peer = self.agent_inspect[vm1_node_ip].get_vna_layer2_route(vm1_vrf_id, mac=vm4_macvlan_mac_addr)['routes'][0]['path_list'][0]['peer']
    ...
    def test_intra_vn_inter_compute_l2(self):
        ...
        mac_cmd = ['ifconfig macvlan1 | grep HWaddr | awk \'{ print $5 }\'']
        vm2_macvlan_mac_addr = list(self.vm2_fixture.run_cmd_on_vm(mac_cmd).values())[0]
        vm3_macvlan_mac_addr = list(self.vm3_fixture.run_cmd_on_vm(mac_cmd).values())[0]
        ...
        try:
            ...
            mac = None
        assert not mac ,"Mac route for macvlan intf of vm3 is present in vm2 EVPN table."

        mac = self.agent_inspect[vm2_node_ip].get_vna_evpn_route(vm2_vrf_id, vxlanid=self.vn2_vxlan_id , mac=vm3_macvlan_mac_addr, ip="0.0.0.0/32")['mac']
        assert  mac == str(self.vn2_vxlan_id)+"-"+vm3_macvlan_mac_addr+"-"+"0.0.0.0/32", "Mac route for macvlan intf of vm3 is absent in vm2 EVPN table. "

        # checking bridge table
        peer = self.agent_inspect[vm2_node_ip].get_vna_layer2_route(vm2_vrf_id, mac=vm3_macvlan_mac_addr)['routes'][0]['path_list'][0]['peer']
        assert peer == "EVPN", "Peer is not EVPN."
        ...
        route = self.get_vrouter_route(self.vm3_macvlan_ip,
                                       ...
                                       inspect_h=inspect_h)
        assert not route, ('Route seen in vrouter for %s' % (self.vm3_macvlan_ip))
    ...
    def test_intra_vn_intra_compute_l2l3(self):
        ...
        assert self.vm1_fixture.ping_to_ip(self.vm4_macvlan_ip.split('/')[0], intf="macvlan1")
        # ping from macvlan1 intf on vm4 to macvlan intf on vm1
        assert self.vm4_fixture.ping_to_ip(self.vm1_macvlan_ip.split('/')[0], intf="macvlan1")
        ...
        mac = self.agent_inspect[vm1_node_ip].get_vna_evpn_route(vm1_vrf_id, vxlanid=self.vn1_vxlan_id , mac=vm4_macvlan_mac_addr, ip="0.0.0.0/32")['mac']
        assert mac == str(self.vn1_vxlan_id)+"-"+vm4_macvlan_mac_addr+"-"+"0.0.0.0/32", "Mac route is absent in EVPN table. "
        ...
        inspect_h = self.agent_inspect[vm1_node_ip]
        route = inspect_h.get_vna_route(vrf_id= vm1_vrf_id, ip=self.vm4_macvlan_ip.split("/")[0])
        assert route, ('No route seen in agent inet table for %s' % (self.vm4_macvlan_ip.split("/")[0]))
        ...
        nh_id = self.inputs.run_cmd_on_server(vm1_node_ip, "docker exec -ti vrouter_vrouter-agent_1 rt --dump %s | grep %s | awk '{print $5}' " % (vm1_vrf_id, route['prefix']+"/"+route['prefix_len']))
        nh_type = self.inputs.run_cmd_on_server(vm1_node_ip, "docker exec -ti vrouter_vrouter-agent_1  nh --get %s | grep Type | awk {'print $2'}" % nh_id).split(":")[1]
        assert nh_type == "Encap" , "Nh type is not Encap."
        encap_data = self.inputs.run_cmd_on_server(vm1_node_ip, "docker exec -ti vrouter_vrouter-agent_1  nh --get %s | grep Encap\ Data" % nh_id).split(":")[1][1:18]
        assert vm4_macvlan_mac_addr.replace(":", " ") == encap_data, "Mac of macvlan intf on vm4 is not present in encap data."

        # checking stitched MAC addr
        stitched_mac_cmd = 'docker exec -ti vrouter_vrouter-agent_1 rt --get %s --vrf %d | awk \'{print $6}\'| grep \':\'' % (self.vm4_macvlan_ip, int(vm1_vrf_id))
        output = self.inputs.run_cmd_on_server(vm1_node_ip, stitched_mac_cmd).split("(")[0]
        assert EUI(output, dialect=mac_unix_expanded) == EUI(vm4_macvlan_mac_addr,dialect=mac_unix_expanded), "Stitched mac address is invalid."
    ...
    def test_intra_vn_inter_compute_l2l3(self):
        ...
        assert self.vm2_fixture.ping_to_ip(self.vm3_macvlan_ip.split('/')[0], intf="macvlan1")
        # ping from macvlan1 intf on vm3 to macvlan intf on vm2
        assert self.vm3_fixture.ping_to_ip(self.vm2_macvlan_ip.split('/')[0], intf="macvlan1")
        ...
        # 0 ip should also be present
        mac = self.agent_inspect[vm2_node_ip].get_vna_evpn_route(vm2_vrf_id, vxlanid=self.vn2_vxlan_id , mac=vm3_macvlan_mac_addr, ip="0.0.0.0/32")['mac']
        assert mac == str(self.vn2_vxlan_id)+"-"+vm3_macvlan_mac_addr+"-"+"0.0.0.0/32", "Mac route is absent in EVPN table. "
        ...
        nh_id = self.inputs.run_cmd_on_server(vm2_node_ip, "docker exec -ti vrouter_vrouter-agent_1 rt --dump %s | grep %s | awk '{print $5}' " % (vm2_vrf_id, route['prefix']+"/"+route['prefix_len']))
        nh_type = self.inputs.run_cmd_on_server(vm2_node_ip, "docker exec -ti vrouter_vrouter-agent_1  nh --get %s | grep Type | awk {'print $2'}" % nh_id).split(":")[1]
        assert nh_type == "Tunnel" , "Nh type is not Tunnel."

        # checking stitched MAC addr
        stitched_mac_cmd = 'docker exec -ti vrouter_vrouter-agent_1 rt --get %s --vrf %d | awk \'{print $6}\'| grep \':\'' % (self.vm3_macvlan_ip, int(vm2_vrf_id))
        output = self.inputs.run_cmd_on_server(vm2_node_ip, stitched_mac_cmd).split("(")[0]
        assert EUI(output, dialect=mac_unix_expanded) == EUI(vm3_macvlan_mac_addr, dialect=mac_unix_expanded), "Stictched mac address is invalid."
    ...
    def test_intra_vn_intra_compute_vlan_l2(self):
        ...
        vm1_vlan_ip =  ".".join(vn2_gw_ip.split(".")[:3]) + "." + str(int(vn2_gw_ip.split(".")[3])+5) + "/32"
        vm1_vlan_macvlan_ip =  ".".join(vn2_gw_ip.split(".")[:3]) + "." + str(int(vn2_gw_ip.split(".")[3])+6) + "/32"
        vm4_vlan_ip =  ".".join(vn2_gw_ip.split(".")[:3]) + "." + str(int(vn2_gw_ip.split(".")[3])+7) + "/32"
        vm4_vlan_macvlan_ip =  ".".join(vn2_gw_ip.split(".")[:3]) + "." + str(int(vn2_gw_ip.split(".")[3])+8) + "/32"
        cmds_vm1 = ['ip link add link eth0 name eth0.100 type vlan id 100',
                    'ip link set dev eth0.100 up',
                    'ip addr add %s dev eth0.100' %(vm1_vlan_ip.split('/')[0] + "/26"),
        ...
        cmds_vm4 = ['ip link add link eth0 name eth0.100 type vlan id 100',
                    'ip link set dev eth0.100 up',
                    'ip addr add %s dev eth0.100' %(vm4_vlan_ip.split('/')[0] + "/26"),
                    'ip link add macvlan1 link eth0.100 type macvlan',
                    'ip link set dev macvlan1 up',
        ...
        vm1_vlan_mac_addr = list(self.vm1_fixture.run_cmd_on_vm(mac_cmd).values())[0]
        vm4_vlan_mac_addr = list(self.vm4_fixture.run_cmd_on_vm(mac_cmd).values())[0]

        parent_vmi_vm1 = self.vnc_lib.virtual_machine_interface_read(id=self.vm1_fixture.get_vmi_id(self.vn1_fixture.vn_fq_name))
        parent_vmi_vm4 = self.vnc_lib.virtual_machine_interface_read(id=self.vm4_fixture.get_vmi_id(self.vn1_fixture.vn_fq_name)) 
        self.setup_vmi(self.vn2_fixture.uuid,parent_vmi=parent_vmi_vm1,api_type="contrail",project_obj=self.project.project_obj,vlan_id="100",mac_address=vm1_vlan_mac_addr,fixed_ips=[{'subnet_id':vm1_vlan_ip.split('/')[1],'ip_address':vm1_vlan_ip.split('/')[0]}])
        self.setup_vmi(self.vn2_fixture.uuid,parent_vmi=parent_vmi_vm4,api_type="contrail",project_obj=self.project.project_obj,vlan_id="100",mac_address=vm4_vlan_mac_addr,fixed_ips=[{'subnet_id':vm4_vlan_ip.split('/')[1],'ip_address':vm4_vlan_ip.split('/')[0]}])

        assert self.vm1_fixture.ping_to_ip(vm4_vlan_macvlan_ip.split('/')[0])
        # ping from macvlan1 intf on vm1 to macvlan intf on vm4
        assert self.vm1_fixture.ping_to_ip(vm4_vlan_macvlan_ip.split('/')[0], intf="macvlan1")
        # ping from macvlan1 intf on vm4 to macvlan intf on vm1
        assert self.vm4_fixture.ping_to_ip(vm1_vlan_macvlan_ip.split('/')[0], intf="macvlan1")
        ...
        try:
            mac = self.agent_inspect[vm1_node_ip].get_vna_evpn_route(vm1_vrf_id, vxlanid=self.vn2_vxlan_id , mac=vm4_macvlan_mac_addr, ip=vm4_vlan_macvlan_ip)['mac']
        except:
            mac = None
        assert not mac ,"Mac route for macvlan intf of vm4 is present in vm1 EVPN table."

        mac = self.agent_inspect[vm1_node_ip].get_vna_evpn_route(vm1_vrf_id, vxlanid=self.vn2_vxlan_id , mac=vm4_macvlan_mac_addr, ip="0.0.0.0/32")['mac']
        assert  mac == str(self.vn2_vxlan_id)+"-"+vm4_macvlan_mac_addr+"-"+"0.0.0.0/32", "Mac route for macvlan intf of vm4 is absent in vm1 EVPN table. "
        ...
        inspect_h = self.agent_inspect[vm1_node_ip]
        route = inspect_h.get_vna_route(vrf_id= vm1_vrf_id, ip=vm4_vlan_macvlan_ip.split("/")[0])
        assert not route , "Route is present in agent inet table." 
    ...
    def test_intra_vn_inter_compute_vlan_l2(self):
        ...
        vm2_vlan_ip =  ".".join(vn1_gw_ip.split(".")[:3]) + "." + str(int(vn1_gw_ip.split(".")[3])+5) + "/32"
        vm2_vlan_macvlan_ip =  ".".join(vn1_gw_ip.split(".")[:3]) + "." + str(int(vn1_gw_ip.split(".")[3])+6) + "/32"
        vm3_vlan_ip =  ".".join(vn1_gw_ip.split(".")[:3]) + "." + str(int(vn1_gw_ip.split(".")[3])+7) + "/32"
        vm3_vlan_macvlan_ip =  ".".join(vn1_gw_ip.split(".")[:3]) + "." + str(int(vn1_gw_ip.split(".")[3])+8) + "/32"
        cmds_vm2 = ['ip link add link eth0 name eth0.100 type vlan id 100',
                    'ip link set dev eth0.100 up',
                    'ip addr add %s dev eth0.100' %(vm2_vlan_ip.split('/')[0] + "/26"),
                    'ip link add macvlan1 link eth0.100 type macvlan',
                    'ip link set dev macvlan1 up',
        ...
        cmds_vm3 = ['ip link add link eth0 name eth0.100 type vlan id 100',
                    'ip link set dev eth0.100 up',
                    'ip addr add %s dev eth0.100' %(vm3_vlan_ip.split('/')[0] + "/26"),
                    'ip link add macvlan1 link eth0.100 type macvlan',
                    'ip link set dev macvlan1 up',
        ...
        mac_cmd = ['ifconfig eth0.100 | grep HWaddr | awk \'{ print $5 }\'']
        vm2_vlan_mac_addr = list(self.vm2_fixture.run_cmd_on_vm(mac_cmd).values())[0]
        vm3_vlan_mac_addr = list(self.vm3_fixture.run_cmd_on_vm(mac_cmd).values())[0]

        parent_vmi_vm2 = self.vnc_lib.virtual_machine_interface_read(id=self.vm2_fixture.get_vmi_id(self.vn2_fixture.vn_fq_name))
        parent_vmi_vm3 = self.vnc_lib.virtual_machine_interface_read(id=self.vm3_fixture.get_vmi_id(self.vn2_fixture.vn_fq_name)) 
        self.setup_vmi(self.vn1_fixture.uuid,parent_vmi=parent_vmi_vm2,api_type="contrail",project_obj=self.project.project_obj,vlan_id="100",mac_address=vm2_vlan_mac_addr,fixed_ips=[{'subnet_id':vm2_vlan_ip.split('/')[1],'ip_address':vm2_vlan_ip.split('/')[0]}])
        self.setup_vmi(self.vn1_fixture.uuid,parent_vmi=parent_vmi_vm3,api_type="contrail",project_obj=self.project.project_obj,vlan_id="100",mac_address=vm3_vlan_mac_addr,fixed_ips=[{'subnet_id':vm3_vlan_ip.split('/')[1],'ip_address':vm3_vlan_ip.split('/')[0]}])

        assert self.vm2_fixture.ping_to_ip(vm3_vlan_macvlan_ip.split('/')[0])
        # ping from macvlan1 intf on vm2 to macvlan intf on vm3
        assert self.vm2_fixture.ping_to_ip(vm3_vlan_macvlan_ip.split('/')[0], intf="macvlan1")
        # ping from macvlan1 intf on vm3 to macvlan intf on vm2
        assert self.vm3_fixture.ping_to_ip(vm2_vlan_macvlan_ip.split('/')[0], intf="macvlan1")
        ...
        try:
            mac = self.agent_inspect[vm2_node_ip].get_vna_evpn_route(vm2_vrf_id, vxlanid=self.vn1_vxlan_id , mac=vm3_macvlan_mac_addr, ip=self.vm3_macvlan_ip)['mac']
        except:
            mac = None
        assert not mac ,"Mac route for macvlan intf of vm3 is present in vm2 EVPN table."

        mac = self.agent_inspect[vm2_node_ip].get_vna_evpn_route(vm2_vrf_id, vxlanid=self.vn1_vxlan_id , mac=vm3_macvlan_mac_addr, ip="0.0.0.0/32")['mac']
        assert  mac == str(self.vn1_vxlan_id)+"-"+vm3_macvlan_mac_addr+"-"+"0.0.0.0/32", "Mac route for macvlan intf of vm3 is absent in vm2 EVPN table. "
    ...
    def test_intra_vn_intra_compute_vlan_l2l3(self):
        ...
        vm1_vrf_id = self.get_vrf_id(self.vn2_fixture, self.vm1_fixture)
        mac = self.agent_inspect[vm1_node_ip].get_vna_evpn_route(vm1_vrf_id, vxlanid=self.vn2_vxlan_id , mac=vm4_macvlan_mac_addr, ip=vm4_vlan_macvlan_ip)['mac']
        assert mac == str(self.vn2_vxlan_id)+"-"+vm4_macvlan_mac_addr+"-"+vm4_vlan_macvlan_ip, "Mac route is absent in EVPN table. "

        # 0 ip should also be present
        mac = self.agent_inspect[vm1_node_ip].get_vna_evpn_route(vm1_vrf_id, vxlanid=self.vn2_vxlan_id , mac=vm4_macvlan_mac_addr, ip="0.0.0.0/32")['mac']
        assert mac == str(self.vn2_vxlan_id)+"-"+vm4_macvlan_mac_addr+"-"+"0.0.0.0/32", "Mac route is absent in EVPN table. "
        ...
        inspect_h = self.agent_inspect[vm1_node_ip]
        route = inspect_h.get_vna_route(vrf_id= vm1_vrf_id, ip=vm4_vlan_macvlan_ip.split("/")[0])
        assert route, ('No route seen in agent inet table for %s' % (vm4_vlan_macvlan_ip.split("/")[0]))
        ...
        # checking stitched MAC addr
        stitched_mac_cmd = 'docker exec -ti vrouter_vrouter-agent_1 rt --get %s --vrf %d | awk \'{print $6}\'| grep \':\'' % (vm4_vlan_macvlan_ip, int(vm1_vrf_id))
        output = self.inputs.run_cmd_on_server(vm1_node_ip, stitched_mac_cmd).split("(")[0]
        assert EUI(output, dialect=mac_unix_expanded) == EUI(vm4_macvlan_mac_addr, dialect=mac_unix_expanded), "Stitched mac address is invalid."
    ...
    def test_intra_vn_inter_compute_vlan_l2l3(self):
        ...
        vm2_node_ip = self.vm2_fixture.vm_node_ip
        vm2_vrf_id = self.get_vrf_id(self.vn1_fixture, self.vm2_fixture)
        mac = self.agent_inspect[vm2_node_ip].get_vna_evpn_route(vm2_vrf_id, vxlanid=self.vn1_vxlan_id , mac=vm3_macvlan_mac_addr, ip=vm3_vlan_macvlan_ip)['mac']
        assert mac == str(self.vn1_vxlan_id)+"-"+vm3_macvlan_mac_addr+"-"+vm3_vlan_macvlan_ip, "Mac route for macvlan1 is absent in EVPN table. "

        # 0 ip should also be present
        mac = self.agent_inspect[vm2_node_ip].get_vna_evpn_route(vm2_vrf_id, vxlanid=self.vn1_vxlan_id , mac=vm3_macvlan_mac_addr, ip="0.0.0.0/32")['mac']
        assert mac == str(self.vn1_vxlan_id)+"-"+vm3_macvlan_mac_addr+"-"+"0.0.0.0/32", "Mac route is absent in EVPN table. "
        ...
        # checking if route for macvlan_ip3 is present in vm2 agent inet table
        inspect_h = self.agent_inspect[vm2_node_ip]
        route = inspect_h.get_vna_route(vrf_id= vm2_vrf_id, ip=vm3_vlan_macvlan_ip.split("/")[0])
        assert route, ('No route seen in inet table for %s' % (vm3_vlan_macvlan_ip.split("/")[0]))

        # checking if route for macvlan_ip3 is present in vm2 vrouter inet table
        route = self.get_vrouter_route(vm3_vlan_macvlan_ip,
                                       vn_fixture=self.vn1_fixture,
                                       inspect_h=inspect_h)
        assert route, ('No route seen in vrouter for %s' % (vm3_vlan_macvlan_ip))
        nh_id = self.inputs.run_cmd_on_server(vm2_node_ip, "docker exec -ti vrouter_vrouter-agent_1 rt --dump %s | grep %s | awk '{print $5}' " % (vm2_vrf_id, route['prefix']+"/"+route['prefix_len']))
        nh_type = self.inputs.run_cmd_on_server(vm2_node_ip, "docker exec -ti vrouter_vrouter-agent_1  nh --get %s | grep Type | awk {'print $2'}" % nh_id).split(":")[1]
        assert nh_type == "Tunnel" , "Nh type is not Tunnel."

        # checking stitched MAC addr
        stitched_mac_cmd = 'docker exec -ti vrouter_vrouter-agent_1 rt --get %s --vrf %d | awk \'{print $6}\'| grep \':\'' % (vm3_vlan_macvlan_ip, int(vm2_vrf_id))
        output = self.inputs.run_cmd_on_server(vm2_node_ip, stitched_mac_cmd).split("(")[0]
        assert EUI(output, dialect=mac_unix_expanded) == EUI(vm3_macvlan_mac_addr, dialect=mac_unix_expanded), "Stictched mac address is invalid."
    ...
    def test_inter_vn_intra_compute_intra_subnet(self):
        ...
        assert self.vm1_fixture.ping_to_ip(self.vm3_macvlan_ip.split('/')[0], intf="macvlan1")
        # ping from macvlan1 intf on vm3 to macvlan intf on vm1
        assert self.vm3_fixture.ping_to_ip(self.vm1_macvlan_ip.split('/')[0], intf="macvlan1")
    ...
    def test_intra_vn_inter_subnet_intra_compute(self):
        ...
        macvlan_ip1 = ".".join(vm1_eth0_ip.split(".")[:3]) + "." + str(int(vm1_eth0_ip.split(".")[3])+5)
        macvlan_ip2 = ".".join(vm2_eth0_ip.split(".")[:3]) + "." + str(int(vm2_eth0_ip.split(".")[3])+5)
    ...
    def test_inter_vn_inter_compute(self):
        ...
        # ping from macvlan1 intf on vm1 to macvlan intf on vm2
        assert self.vm1_fixture.ping_to_ip(self.vm2_macvlan_ip.split('/')[0], intf="macvlan1")
        # ping from macvlan1 intf on vm4 to macvlan intf on vm1
        assert self.vm2_fixture.ping_to_ip(self.vm1_macvlan_ip.split('/')[0], intf="macvlan1")
    ...
    def test_delete_macvlan_intf(self):
        ...
        time.sleep(60)
        # from vm1 to mac4 intf
        assert not self.vm1_fixture.ping_to_ip(self.vm4_macvlan_ip.split('/')[0])
        # checking evpn table
        vm1_node_ip = self.vm1_fixture.vm_node_ip
        vm1_vrf_id = self.get_vrf_id(self.vn1_fixture, self.vm1_fixture)
        try:
        ...
        inspect_h = self.agent_inspect[vm1_node_ip]
        route = inspect_h.get_vna_route(vrf_id= vm1_vrf_id, ip=self.vm4_macvlan_ip.split("/")[0])
        assert not route, ('Route seen in agent inet table for %s' % (self.vm4_macvlan_ip.split("/")[0]))

        # checking if vm4_macvlan_ip is present in vm1 vrouter inet table
        # checking route in vrouter got deleted 
        route_ppl_cmd = 'docker exec -ti vrouter_vrouter-agent_1 rt --dump %d | grep %s | awk \'{print $2}\'' % (int(vm1_vrf_id), self.vm4_macvlan_ip.split('/')[0])
    ...
    def test_delete_vmi(self):
        ...
        try:
            ...
            mac = None
        assert not mac, "Mac route is present in EVPN table. "
    ...
    def test_delete_vlan_intf(self):
        ...
        vm1_vrf_id = self.get_vrf_id(self.vn2_fixture, self.vm1_fixture)
        try:
            mac = self.agent_inspect[vm1_node_ip].get_vna_evpn_route(vm1_vrf_id, vxlanid=self.vn1_vxlan_id , mac=vm4_macvlan_mac_addr, ip=vm4_vlan_macvlan_ip)['mac']
        except TypeError:
            mac = None
        assert not mac, "Mac route for macvlan4 is present in EVPN table. "
        ...
        inspect_h = self.agent_inspect[vm1_node_ip]
        route = inspect_h.get_vna_route(vrf_id= vm1_vrf_id, ip=vm4_vlan_macvlan_ip.split("/")[0])
        assert not route, ('Route seen in vrouter for %s' % (vm4_vlan_macvlan_ip.split("/")[0]))

        # checking Vrouter inet table in vm1 for vm4_vlan_macvlan_ip
        # checking route in vrouter got deleted 
        route_ppl_cmd = 'docker exec -ti vrouter_vrouter-agent_1 rt --dump %d | grep %s | awk \'{print $2}\'' % (int(vm1_vrf_id), vm4_vlan_macvlan_ip.split('/')[0])
        output = self.inputs.run_cmd_on_server(vm1_node_ip, route_ppl_cmd)
        assert output != "32", "Route not deleted in vrouter inet table."
        return True
    ...
    def test_move_ip_locally_l2(self):
        ...
        vm5_fixture = self.create_vm(vn_fixture=self.vn1_fixture, image_name='ubuntu', vm_name=vm5_name, node_name=self.node1)
        ...
        vm5_to_macvlanip_ping = self.start_ping(vm5_fixture, dst_ip=self.vm1_macvlan_ip.split('/')[0])
        vm5_node_ip = vm5_fixture.vm_node_ip
        time.sleep(60)
        flow = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1 flow --match %s | grep %s | grep -v \"Listing\"" % (self.vm1_macvlan_ip.split('/')[0], self.vm1_macvlan_ip.split('/')[0]))
        assert flow, "Flow not created."
        mac1_nh_id = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1 flow --match %s| grep -m 2 S\(nh\)| awk {'print $7'}" % self.vm1_macvlan_ip.split('/')[0]).split(":")[1].split(",")[0]
        mac1_oif = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1  nh --get %s | grep Oif | awk {'print $2'}" % mac1_nh_id).split(":")[1]
        self.vn1_fixture.add_forwarding_mode(project_fq_name=self.inputs.project_fq_name, vn_name=self.vn1_name, forwarding_mode = "l2")
      
        # checking evpn table
        vm5_vrf_id = self.get_vrf_id(self.vn1_fixture, vm5_fixture)
        try:
            mac = self.agent_inspect[vm5_node_ip].get_vna_evpn_route(vm5_vrf_id, vxlanid=self.vn1_vxlan_id , mac=vm1_macvlan_mac_addr, ip=self.vm1_macvlan_ip)['mac']
        except:
            mac = None
        assert not mac ,"Mac route for macvlan intf of vm5 is present in vm1 EVPN table."

        mac = self.agent_inspect[vm5_node_ip].get_vna_evpn_route(vm5_vrf_id, vxlanid=self.vn1_vxlan_id , mac=vm1_macvlan_mac_addr, ip="0.0.0.0/32")['mac']
        assert  mac == str(self.vn1_vxlan_id)+"-"+vm1_macvlan_mac_addr+"-"+"0.0.0.0/32", "Mac route for macvlan intf of vm5 is absent in vm1 EVPN table. "

        # checking bridge table
        peer = self.agent_inspect[vm5_node_ip].get_vna_layer2_route(vm5_vrf_id, mac=vm1_macvlan_mac_addr)['routes'][0]['path_list'][0]['peer']
        assert peer == "EVPN", "Peer is not EVPN."
        self.vn1_fixture.add_forwarding_mode(project_fq_name=self.inputs.project_fq_name, vn_name=self.vn1_name, forwarding_mode = "l2_l3")
        cmds_vm4 = ['ip link add macvlan1 link eth0 type macvlan',
                    'ip link set dev macvlan1 up',
        ...
        flow = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1 flow --match %s | grep %s | grep -v \"Listing\"" % (self.vm1_macvlan_ip.split('/')[0], self.vm1_macvlan_ip.split('/')[0]))
        assert flow, "Flow not created."
        mac2_nh_id = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1 flow --match %s | grep -m 3 S\(nh\) | tail -n 1 | awk {'print $7'}" % self.vm1_macvlan_ip.split('/')[0]).split(":")[1].split(",")[0]
        mac2_oif = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1  nh --get %s | grep Oif | awk {'print $2'}" % mac2_nh_id).split(":")[1]
        assert mac1_oif != mac2_oif , "Oif has not changed."
        encap_data = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1  nh --get %s | grep Encap\ Data" % mac2_nh_id).split(":")[1][1:18]
        assert vm4_macvlan_mac_addr.replace(":", " ") == encap_data, "Mac2 is not updated in encap data."

        self.vn1_fixture.add_forwarding_mode(project_fq_name=self.inputs.project_fq_name, vn_name=self.vn1_name, forwarding_mode = "l2")
        # checking evpn table if MAC2 is added in vm5
        try:
            mac = self.agent_inspect[vm5_node_ip].get_vna_evpn_route(vm5_vrf_id, vxlanid=self.vn1_vxlan_id , mac=vm4_macvlan_mac_addr, ip=self.vm1_macvlan_ip)['mac']
        except TypeError:
            mac = None
        assert not mac, "Mac route for macvlan4 is absent in EVPN table. "

        # checking if macvlan4 is added to evpn table
        mac = self.agent_inspect[vm5_node_ip].get_vna_evpn_route(vm5_vrf_id, vxlanid=self.vn1_vxlan_id , mac=vm4_macvlan_mac_addr, ip="0.0.0.0/32")['mac']
        assert mac == str(self.vn1_vxlan_id)+"-"+vm4_macvlan_mac_addr+"-"+"0.0.0.0/32", "Mac route for macvlan4 is absent in EVPN table. "
        ...
        peer = self.agent_inspect[vm5_node_ip].get_vna_layer2_route(vm5_vrf_id, mac=vm4_macvlan_mac_addr)['routes'][0]['path_list'][0]['peer']
        assert peer == "EVPN", "Peer is not EVPN."
    ...
    def test_move_ip_across_computes_l2(self):
        ...
        '''
        vm5_name = get_random_name('vm5')
        vm5_fixture = self.create_vm(vn_fixture=self.vn2_fixture, image_name='ubuntu', vm_name=vm5_name, node_name=self.node1)
        assert vm5_fixture.wait_till_vm_is_up()
        ...
        vm5_to_macvlanip_ping = self.start_ping(vm5_fixture, dst_ip=self.vm2_macvlan_ip.split('/')[0])
        vm5_node_ip = vm5_fixture.vm_node_ip
        time.sleep(60)
        flow = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1 flow --match %s | grep %s | grep -v \"Listing\"" % (self.vm2_macvlan_ip.split('/')[0], self.vm2_macvlan_ip.split('/')[0]))
        assert flow, "Flow not created."
        mac2_nh_id = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1 flow --match %s| grep -m 2 S\(nh\)| awk {'print $7'}" % self.vm2_macvlan_ip.split('/')[0]).split(":")[1].split(",")[0]
        mac2_oif = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1  nh --get %s | grep Oif | awk {'print $2'}" % mac2_nh_id).split(":")[1]

        self.vn2_fixture.add_forwarding_mode(project_fq_name=self.inputs.project_fq_name, vn_name=self.vn2_name, forwarding_mode = "l2")
        # checking evpn table
        vm5_vrf_id = self.get_vrf_id(self.vn2_fixture, vm5_fixture)
        mac = self.agent_inspect[vm5_node_ip].get_vna_evpn_route(vm5_vrf_id, vxlanid=self.vn2_vxlan_id , mac=vm2_macvlan_mac_addr, ip="0.0.0.0/32")['mac']
        assert mac == str(self.vn2_vxlan_id)+"-"+vm2_macvlan_mac_addr+"-"+"0.0.0.0/32", "Mac route for macvlan1 is absent in EVPN table. "

        # checking bridge table
        peer = self.agent_inspect[vm5_node_ip].get_vna_layer2_route(vm5_vrf_id, mac=vm2_macvlan_mac_addr)['routes'][0]['path_list'][0]['peer']
        assert peer == "EVPN", "Peer is not EVPN."
        ...
        flow = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1 flow --match %s | grep %s | grep -v \"Listing\"" % (self.vm2_macvlan_ip.split('/')[0], self.vm2_macvlan_ip.split('/')[0]))
        assert flow, "Flow not created."
        mac3_nh_id = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1 flow --match %s | grep -m 3 S\(nh\) | tail -n 1 | awk {'print $7'}" % self.vm2_macvlan_ip.split('/')[0]).split(":")[1].split(",")[0]
        mac3_oif = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1  nh --get %s | grep Oif | awk {'print $2'}" % mac3_nh_id).split(":")[1]
        assert mac2_oif != mac3_oif , "Oif has not changed."
        encap_data = self.inputs.run_cmd_on_server(vm5_node_ip, "docker exec -ti vrouter_vrouter-agent_1  nh --get %s | grep Encap\ Data" % mac3_nh_id).split(":")[1][1:18]
        assert vm3_macvlan_mac_addr.replace(":", " ") == encap_data, "Mac2 is not updated in encap data."

        self.vn2_fixture.add_forwarding_mode(project_fq_name=self.inputs.project_fq_name, vn_name=self.vn2_name, forwarding_mode = "l2")
        # checking evpn table for macvlan1 intf on vm3
        mac = self.agent_inspect[vm5_node_ip].get_vna_evpn_route(vm5_vrf_id, vxlanid=self.vn2_vxlan_id , mac=vm3_macvlan_mac_addr, ip="0.0.0.0/32")['mac']
        assert mac == str(self.vn2_vxlan_id)+"-"+vm3_macvlan_mac_addr+"-"+"0.0.0.0/32", "Mac route for macvlan4 is absent in EVPN table. "
        ...
        # checking bridge table
        peer = self.agent_inspect[vm5_node_ip].get_vna_layer2_route(vm5_vrf_id, mac=vm3_macvlan_mac_addr)['routes'][0]['path_list'][0]['peer']
        assert peer == "EVPN", "Peer is not EVPN."
    ...
    def test_move_ip_locally_l2l3(self):
        ...
        route = inspect_h.get_vna_route(vrf_id= vm5_vrf_id, ip=self.vm1_macvlan_ip.split("/")[0])
        ...
        # checking stitched MAC addr
        stitched_mac_cmd = 'docker exec -ti vrouter_vrouter-agent_1 rt --get %s --vrf %d | awk \'{print $6}\'| grep \':\'' % (self.vm1_macvlan_ip, int(vm5_vrf_id))
        output = self.inputs.run_cmd_on_server(vm5_node_ip, stitched_mac_cmd).split("(")[0]
        assert EUI(output, dialect=mac_unix_expanded) == EUI(vm1_macvlan_mac_addr,dialect=mac_unix_expanded), "Stitched mac address is invalid."
        ...
        # deleting ip in arp cache to improve the time in which arp gets updated
    ...
    def test_move_ip_across_computes_l2l3(self):
        ...
        '''
        vm5_name = get_random_name('vm5')
        vm5_fixture = self.create_vm(vn_fixture=self.vn2_fixture, image_name='ubuntu', vm_name=vm5_name, node_name=self.node2)
        assert vm5_fixture.wait_till_vm_is_up()
        cmds_vm2 = ['ip link add macvlan1 link eth0 type macvlan',
                    'ip link set dev macvlan1 up',
        ...
        vm5_node_ip = vm5_fixture.vm_node_ip
        vm5_vrf_id = self.get_vrf_id(self.vn2_fixture, vm5_fixture)
        mac = self.agent_inspect[vm5_node_ip].get_vna_evpn_route(vm5_vrf_id, vxlanid=self.vn2_vxlan_id , mac=vm2_macvlan_mac_addr, ip=self.vm2_macvlan_ip)['mac']
        ...
        inspect_h = self.agent_inspect[vm5_node_ip]
        route = inspect_h.get_vna_route(vrf_id= vm5_vrf_id, ip=self.vm2_macvlan_ip.split("/")[0])
        ...
        # checking stitched MAC addr
        stitched_mac_cmd = 'docker exec -ti vrouter_vrouter-agent_1 rt --get %s --vrf %d | awk \'{print $6}\'| grep \':\'' % (self.vm2_macvlan_ip, int(vm5_vrf_id))
        output = self.inputs.run_cmd_on_server(vm5_node_ip, stitched_mac_cmd).split("(")[0]
        assert EUI(output, dialect=mac_unix_expanded) == EUI(vm2_macvlan_mac_addr,dialect=mac_unix_expanded), "Stitched mac address is invalid."
        cmd = ['ip link set dev macvlan1 down']
        self.vm2_fixture.run_cmd_on_vm(cmd, as_sudo=True)
        cmds_vm3 = ['ip link add macvlan1 link eth0 type macvlan',
                    'ip link set dev macvlan1 up',
        ...
        assert vm5_fixture.ping_to_ip(self.vm2_macvlan_ip.split('/')[0])
        # checking evpn table
        mac = self.agent_inspect[vm5_node_ip].get_vna_evpn_route(vm5_vrf_id, vxlanid=self.vn2_vxlan_id , mac=vm3_macvlan_mac_addr, ip=self.vm2_macvlan_ip)['mac']
        ...
        stitched_mac_cmd = 'docker exec -ti vrouter_vrouter-agent_1 rt --get %s --vrf %d | awk \'{print $6}\'| grep \':\'' % (self.vm2_macvlan_ip, int(vm5_vrf_id))
        output = self.inputs.run_cmd_on_server(vm5_node_ip, stitched_mac_cmd).split("(")[0]
        assert EUI(output, dialect=mac_unix_expanded) == EUI(vm3_macvlan_mac_addr,dialect=mac_unix_expanded), "Stitched mac address is invalid."
    ...
    def test_dynamically_disable_maciplearningflag(self):
        ...
        cmds_vm4 = ['ip link add macvlan1 link eth0 type macvlan',
                    ...
                    'ip addr add %s dev macvlan1' % (self.vm4_macvlan_ip.split('/')[0] + "/26")]
        self.vm4_fixture.run_cmd_on_vm(cmds_vm4, as_sudo=True)
        assert self.vn1_fixture.set_mac_ip_learning(mac_ip_learning_enable=False)
    ...
    def test_change_fwding_mode(self):
        ...
        route = self.get_vrouter_route(self.vm4_macvlan_ip,
                                       vn_fixture=self.vn1_fixture,
                                       inspect_h=inspect_h)
        assert not route, ('No route seen in vrouter for %s' % (self.vm4_macvlan_ip))
    ...
    def test_fifty_macvlans(self):
        ...
        '''
        for i in range(1,51):
            macvlan_ip = ".".join(self.vm1_eth0_ip.split(".")[:3]) + "." + str(int(self.vm1_eth0_ip.split(".")[3])+i+5)
    ...
    def test_vrouter_agent_restart(self):
        ...
        self.inputs.run_cmd_on_server(vm2_node_ip, "docker restart vrouter_vrouter-agent_1")
        time.sleep(30)
        mac = self.agent_inspect[vm2_node_ip].get_vna_evpn_route(vm2_vrf_id, vxlanid=self.vn2_vxlan_id , mac=vm3_macvlan_mac_addr, ip="0.0.0.0/32")['mac']
        assert  mac == str(self.vn2_vxlan_id)+"-"+vm3_macvlan_mac_addr+"-"+"0.0.0.0/32", "Mac route for macvlan intf of vm3 is absent in vm2 EVPN table. "
    ...
    def test_in_l3_mode(self):
        ...
        self.vn1_fixture.add_forwarding_mode(project_fq_name=self.inputs.project_fq_name, vn_name=self.vn1_name, forwarding_mode = "l3")

        # from vm1 to mac4 intf
        assert not self.vm1_fixture.ping_to_ip(self.vm4_macvlan_ip.split('/')[0])
        # ping from macvlan1 intf on vm1 to macvlan intf on vm4
        assert not self.vm1_fixture.ping_to_ip(self.vm4_macvlan_ip.split('/')[0], intf="macvlan1")
        # ping from macvlan1 intf on vm4 to macvlan intf on vm1
        assert not self.vm4_fixture.ping_to_ip(self.vm1_macvlan_ip.split('/')[0], intf="macvlan1")
    ...
    def test_bfd_on_vsrx_1(self):
        ...
        Test steps:
               1. Create a vsrx vm.
               2. Create and attach BFD health check to the vsrx vm.
               3. Create a static route to the vsrx vm
               4. Configure BFD on vsrx  
        Pass criteria:
               1. Verify BFD session is up.
               2. Remove the target ip and mac from vsrx and verify BFD session failed. Target routes are deleted from inet and evpn tables.
        Maintainer : ybadaya@juniper.net
        '''
        vn_name =  get_random_name('vsrx_vm_vn')
        vn_subnet = [get_random_cidr(af="v4")]
        vn_fixture = self.create_vn(vn_name, vn_subnet)
        vsrx_vm_fixture = self.create_vm(vn_fixture=vn_fixture, image_name='vsrx', vm_name="vsrx_vm1", node_name=self.node1)
        assert vsrx_vm_fixture.wait_till_vm_is_up()

        vn_vxlan_id = vn_fixture.get_vxlan_id()
        gw_ip = vn_fixture.get_subnets()[0]['gateway_ip']
        target_ip =  ".".join(gw_ip.split(".")[:3]) + "." + str(int(gw_ip.split(".")[3])+2) + "/" + vn_subnet[0].split('/')[1]
        lo_ip =  ".".join(gw_ip.split(".")[:3]) + "." + str(int(gw_ip.split(".")[3])+3) + "/32"
        target_mac = "00:11:22:33:44:55"

        # creating BFD health check
        shc_fixture = self.create_hc(hc_type='vn-ip-list', probe_type='BFD', target_ip_list=[target_ip])
        self.attach_shc_to_vmi(shc_fixture, vsrx_vm_fixture) 

        # attaching a routing table to vm
        static_table_handle = ContrailVncApi(self.vnc_lib, self.logger)
        rt = static_table_handle.create_route_table(
                prefixes=[vn_subnet[0]],
                name="vsrx_rt",
                next_hop=target_ip,
        ...
        static_table_handle.bind_network_route_table_to_vn(
                vn_uuid=vn_fixture.uuid,
                nw_route_table_obj=rt)

        self.config_bfd_on_vsrx(mac=target_mac,
                               intf_ip=target_ip,
                               dest_vm=vsrx_vm_fixture.vm_node_ip,
                               gw_ip=gw_ip,
                               lo_ip=lo_ip)
        assert self.verify_bfd_packets(vsrx_vm_fixture, vn_fixture), 'BFD packets not seen.'
        
        # Removing target ip and mac
        self.remove_target_ip_mac_on_vsrx(dest_vm=vsrx_vm_fixture.vm_node_ip, mac=target_mac, intf_ip=target_ip)

        # Checking if BFD failed 
        assert not self.verify_bfd_packets(vsrx_vm_fixture, vn_fixture), 'BFD packets are seen.'

        # Check if target ip routes are deleted
        # checking evpn table
        vsrx_vm_node_ip = vsrx_vm_fixture.vm_node_ip
        vsrx_vm_vrf_id = self.get_vrf_id(vn_fixture, vsrx_vm_fixture)
        mac = self.agent_inspect[vsrx_vm_node_ip].get_vna_evpn_route(vsrx_vm_vrf_id, vxlanid=vn_vxlan_id , mac=target_mac, ip=target_ip)['mac']
        assert mac != str(vn_vxlan_id)+"-"+target_mac+"-"+target_ip, "Mac route for target_ip is present in EVPN table. "

        # checking if route for target_ip is in vsrx_vm inet table route
        inspect_h = self.agent_inspect[vsrx_vm_node_ip]
        route = inspect_h.get_vna_route(vrf_id= vsrx_vm_vrf_id, ip=target_ip.split("/")[0])
        assert not route, ('Route seen in inet table for %s' % (target_ip.split("/")[0]))

        self.detach_shc_from_vmi(shc_fixture, vsrx_vm_fixture)
        return True
    # end test_bfd_on_vsrx_1

    @test.attr(type=['suite1', 'upgrade','vrouter_gw', 'vcenter_compute', 'ci_contrail_go_kolla_ocata_sanity'])
    @preposttest_wrapper
    def test_bfd_on_vsrx_2(self):
        '''
        Description: Verify that target IP routes are not affected when health check is detached from VN.
        Test steps:
               1. Create a vsrx vm.
               2. Create and attach BFD health check to vsrx vm  with target_ip_all as True.
               3. Create a static route to the vsrx vm
               4. Configure BFD on vsrx
        Pass criteria:
               1. Verify BFD session is up.
               3. Detach hc and verify target routes are not affected.
        Maintainer : ybadaya@juniper.net
        '''
        vn_name =  get_random_name('vsrx_vm_vn')
        vn_subnet = [get_random_cidr(af="v4")]
        ...
        # creating BFD health check
        shc_fixture = self.create_hc(hc_type='vn-ip-list', probe_type='BFD', target_ip_all=True)
        self.attach_shc_to_vmi(shc_fixture, vsrx_vm_fixture) 
        ...
        # Check if target ip routes are present
        # checking evpn table
        vsrx_vm_node_ip = vsrx_vm_fixture.vm_node_ip
        vsrx_vm_vrf_id = self.get_vrf_id(vn_fixture, vsrx_vm_fixture)
        mac = self.agent_inspect[vsrx_vm_node_ip].get_vna_evpn_route(vsrx_vm_vrf_id, vxlanid=vn_vxlan_id , mac=target_mac, ip=target_ip)['mac']
        assert mac == str(vn_vxlan_id)+"-"+target_mac+"-"+target_ip, "Mac route for target_ip is absent in EVPN table. "

        # checking if route for target_ip is in vsrx_vm inet table route
        inspect_h = self.agent_inspect[vsrx_vm_node_ip]
        route = inspect_h.get_vna_route(vrf_id= vsrx_vm_vrf_id, ip=target_ip.split("/")[0])
        assert  route, ('No route seen in inet table for %s' % (target_ip.split("/")[0]))
        return True
    # end test_bfd_on_vsrx_2
