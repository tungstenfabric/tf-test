...
class TestIpMacLearning(BaseVrouterTest, BaseIpMacLearningTest, BaseHC):
    ...
    @test.attr(type=['suite1', 'upgrade', 'vrouter_gw',
                     'vcenter_compute', 'ci_contrail_go_kolla_ocata_sanity'])
    @preposttest_wrapper
    ...
    def test_detach_hc_vsrx(self):
        ...
        assert vm_state
        gw_ip = vn1_fixture.get_subnets()[0]['gateway_ip']
        target_ip =  ".".join(gw_ip.split(".")[:3]) + "." + str(int(gw_ip.split(".")[3])+10) + "/" + vn1_fixture.get_cidrs()[0].split('/')[1]
        for ip in vm1_fixture.vm_ips:
            if ".".join(vn1_fixture.get_subnets()[0]['cidr'].split(".")[:3]) == ".".join(ip.split(".")[:3]):
                lo_ip = ip
                break
        ...
        # checking evpn table for target mac-ip
        evpn_route = self.agent_inspect[vsrx_vm_node_ip].get_vna_evpn_route(vsrx_vm_vrf_id, vxlanid=vn1_vxlan_id , mac=target_mac, ip=target_ip.split('/')[0]+"/32")['mac']
        assert evpn_route == str(vn1_vxlan_id)+"-"+target_mac+"-"+target_ip.split('/')[0]+"/32", "Mac route for target_ip is absent in EVPN table. "

        # checking if route for target_ip is in vsrx_vm inet table route
        inspect_h = self.agent_inspect[vsrx_vm_node_ip]
        route = inspect_h.get_vna_route(vrf_id= vsrx_vm_vrf_id, ip=target_ip.split("/")[0])
        assert route, ('Route seen in inet table for %s' % (target_ip.split("/")[0]))
