...
def determine_containerization_engine(self):
...
class TestInputs(with_metaclass(Singleton, object)):
    ...
    def __init__(self, input_file, logger=None):
        ...
        self.docker = self.determine_containerization_engine()
        self.tor_agent_data = {}
        self.sriov_data = {}
        self.dpdk_data = {}
...
#            introspect_certbundle = self.introspect_cafile
    ...
    def _check_containers(self, host_dict):
        ...
        if  host_dict.get('type', None) == 'esxi':
            return
        cmd = 'docker ps -a | grep -v "/pause\|/usr/bin/pod\|nova_api_\|contrail.*init\|init.*contrail\|provisioner" | awk \'{print $NF}\''
