...
class TestFabricTrans(BaseFabricTest):
    ...
    def test_bgp_router(self):
        status = self._verify_log_entry("Existing Fabric Onboarding")
        assert status, "Fabric onboarding log missing"
        status = self._verify_log_entry("Role Assignment")
        assert status, "Role Assignment log missing"
        pr_fixture = self.devices[0]
        pr_obj = self.vnc_h.read_physical_router(id=pr_fixture.uuid)
        bgp_uuid = pr_obj.bgp_router_refs[0].get('uuid')
        bgp_obj =  self.vnc_h._vnc.bgp_router_read(id=bgp_uuid)
        ...
        self.vnc_h._vnc.bgp_router_update(bgp_obj)
