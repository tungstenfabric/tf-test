...
class TestFabricCollapsedSpine(BaseFabricTest):
    def is_test_applicable(self):
        ...
        for device_dict in list(self.inputs.physical_routers_data.values()):
            if 'collapsed-spine' in (device_dict.get('rb_roles') or []) and device_dict.get('role') == 'spine':
                ...
                self.rb_roles[device_dict['name']] = ['Collapsed-Spine', 'Route-Reflector', 'CRB-Gateway']
    ...
    def configure_fabric_for_collapsed_spine(self, intravn=False):
        ...
        self.instances, self.vns, self.lr_devices, self.bms_ips = [], [], [], []
        i = 0
        for device_dict in self.c_spines:
            ...
            port_fixture = self.create_vmi(fabric_fixture=self.fabric, interfaces=[interface], name=vpg.get_name(),
                                           vnid=vn.get_uuid(), vlan_id=i+10, vlan_tag=i+10)
            ...
            self.configure_l2_vlan(pi_name=up_ifc, vlan_name=vlan, vlan_id=i+10, host=mgmt_ip, up_link=True,
                                   username=ssh_username, password=ssh_password, name=l2_name)
            self.configure_l2_vlan(pi_name=down_ifc, vlan_name=vlan, vlan_id=i+10, host=mgmt_ip,
                                   username=ssh_username, password=ssh_password, name=l2_name)

            # create namespace on bms
            bms_ip = self.get_bms_ip(vn)
            bms = self.create_bms(bms_name=bms, vn_fixture=vn, tor_port_vlan_tag=i+10, verify_bms=False,
                                  fabric_fixture=self.fabric, vmi_create=False, bms_ip=bms_ip, vlan_id=i+10)
            self.instances.append(bms)
            if not intravn:
                i += 10

    def get_bms_ip(self, vn):
        bms_ip = vn.get_an_ip(random_ip=True)
        if bms_ip not in self.bms_ips:
            self.bms_ips.append(bms_ip)
        else:
            bms_ip = self.get_bms_ip(vn)
        return bms_ip
